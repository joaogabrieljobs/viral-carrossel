...
describe('Gate do proxy Anthropic (audit-produto §crítico 3)', () => {
  const stubUpstream = () => {
  ...
  it('assinante com sessão usa a chave do host', async () => {
    ...
    const cookie = `${COOKIE_NAME}=${encodeURIComponent(createAccessToken({ customerId: 'cus_ok', email: 'ok@teste.exemplo' }))}`;
    const res = makeRes();
  ...
  it('chave própria (BYOK) passa sem sessão', async () => {
    const spy = stubUpstream();
    const res = makeRes();
