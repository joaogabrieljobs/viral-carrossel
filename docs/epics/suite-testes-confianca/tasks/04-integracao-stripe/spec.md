...
## 2. Contexto herdado
- Contratos TDD §5. Código: `handleCheckout` (400 e-mail inválido; cliente novo via `customers.create`; já-ativo → `alreadyActive`+`requireLogin` **sem** cookie — login só via Google), `handleConfirm` (402 não paga; 400 modo errado; cookie no sucesso), `handlePortal` (401 sem cookie), `api/stripe/webhook.js` (edge, `constructEventAsync` + SubtleCrypto, exige `whsec_`).
...
- [ ] CA-01: checkout — e-mail inválido 400; novo cliente cria customer e retorna url; já ativo → alreadyActive + requireLogin **sem** Set-Cookie; 405
